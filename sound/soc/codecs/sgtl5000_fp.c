...
static const DECLARE_TLV_DB_SCALE(mic_20db_boost, 0, 2000, 0);
 ...
 * 1. vddd provided by external as per errata
...
static int sgtl5000_set_power_regs(struct snd_soc_codec *codec)
{
	...
	if (vdda < 1620 || vddio < 1620 || vddd < 1100) {
		dev_err(codec->dev,
			"low voltage vdda %dmV vddio %dmV vddd %dmV\n",
			vdda, vddio, vddd);
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (np) {
		...
		if (!of_property_read_u32(np,"VDDA-supply", &value)) {
			sgtl5000->vdda_voltage = value;
		} else {
			sgtl5000->vdda_voltage = 0;
		}
		if (!of_property_read_u32(np,"VDDIO-supply", &value)) {
			sgtl5000->vddio_voltage = value;
		} else {
			sgtl5000->vddio_voltage = 0;
		}
		if (!of_property_read_u32(np,"VDDD-supply", &value)) {
			sgtl5000->vddd_voltage = value;
		} else {
			sgtl5000->vddd_voltage = 0;
		}
	}
